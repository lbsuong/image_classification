...
def dot_kernel(A, B, C):
    ...
    if row >= C.shape[0] or col >= C.shape[1]:
        return
    C[row, col] = 0
    for col_A in range(A.shape[1]):
        C[row, col] += A[row, col_A] * B[col_A, col]


def dot(A, B, blockSize=(16, 16)):
    '''
    for host calling
    '''
    C = np.empty(shape=(A.shape[0], B.shape[1]))
    blockspergrid_y = math.ceil(C.shape[1] / blockSize[1])
    blockspergrid_x = math.ceil(C.shape[0] / blockSize[0])
    gridSize = (blockspergrid_x, blockspergrid_y)
    dot_kernel[gridSize, blockSize](A, B, C)
    # cuda.synchronize()
    return C
...
def normalize(X, blockSize=(32, 32)):
    '''
    for host calling
    '''
    ...
    gridSize = (blockspergrid_x, blockspergrid_y)
    normalize_kernel[gridSize, blockSize](X)
    return X
...
def softmax_backprop(d_L_d_out, learningRate, weights, biases, maxpoolFlattenedOutputs, maxpoolOutputsShape, preSoftmax):
    ...
    for i, gradient in enumerate(d_L_d_out):
        ...
        for k in range(len(e_preSoftmax)):
            d_out_d_preSoftmax[k] = (-e_preSoftmax[i]
                                     * e_preSoftmax[k]) / (S ** 2)
        d_out_d_preSoftmax[i] = e_preSoftmax[i] * \
            (S - e_preSoftmax[i]) / (S ** 2)
        ...
        d_L_d_w = dot(d_L_d_preSoftmax.reshape(len(d_L_d_preSoftmax), 1),
                      d_preSoftmax_d_w.reshape(1, len(d_preSoftmax_d_w)))
        ...
        d_L_d_inputs = dot(d_L_d_preSoftmax.reshape(
            1, len(d_L_d_preSoftmax)), d_preSoftmax_d_inputs)
    ...
    for node in range(d_L_d_out.shape[0]):
        for row in range(d_L_d_out.shape[1]):
            for col in range(d_L_d_out.shape[2]):
                _, (convForwardOutputs_maxRow, convForwardOutputs_maxCol) = matrix_max(
                    convForwardOutputs[node, row * maxpoolSize: maxpoolSize * (row + 1), col * maxpoolSize: maxpoolSize * (col + 1)])
                d_L_d_inputs[node, convForwardOutputs_maxRow + row * maxpoolSize,
                             convForwardOutputs_maxCol + col * maxpoolSize] = d_L_d_out[node, row, col]
    return d_L_d_inputs
            ...
            for filter_col in range(convFilters.shape[2]):
                for d_L_d_out_row in range(d_L_d_out.shape[1]):
                    for d_L_d_out_col in range(d_L_d_out.shape[2]):
                        d_L_d_filters[node, filter_row, filter_col] += d_L_d_out[node, d_L_d_out_row,
                                                                                 d_L_d_out_col] * normalizedImage[d_L_d_out_row + filter_row, d_L_d_out_col + filter_col]
    for node in range(convFilters.shape[0]):
...
def train(trainImages, trainLabels, learningRate, convFilters1, convFilters2, maxpoolSize, softmaxWeights, softmaxBiases):
    ...
    for i in range(trainImages.shape[0]):
        ...
        preSoftmax, postSoftmax = softmax_forward(
        ...
        # Lan truyền ngược.
        gradient = softmax_backprop(gradient, learningRate, softmaxWeights,
                                    softmaxBiases, maxpoolOutputs.flatten(), maxpoolOutputs.shape, preSoftmax)
        gradient = maxpool_backprop(gradient, convOutputs, maxpoolSize)
        gradient = conv_backprop(
            gradient, learningRate, convFilters1, trainImages[i])
...
def main():
    ...
    for e in range(epoch):
        ...
        trainingLoss, trainingAccuracy = train(
            normalizeTrainImages[trainingShuffler], trainLabels[trainingShuffler], learningRate, convFilters, convFilters, maxpoolSize, softmaxWeights, softmaxBiases)
        validationLoss, validationAccuracy = validate(
            normalizeValidateImages[validationShuffler], validateLabels[validationShuffler], convFilters, convFilters, maxpoolSize, softmaxWeights, softmaxBiases)
        epochEnd = time.time()
        print("{second}s - loss: {trainingLoss:.4f} - accuracy: {trainingAccuracy:.4f} - val_loss: {validationLoss:.4f} - val_accuracy: {validationAccuracy:.4f}".format(
            second=int(epochEnd-epochStart), trainingLoss=trainingLoss, trainingAccuracy=trainingAccuracy, validationLoss=validationLoss, validationAccuracy=validationAccuracy))
    stop = time.time()
