...
def main():
    ...
    numNode = 10
    softmaxWeightsLength = (math.ceil((trainImages.shape[1] - convFiltersH + 1) / maxpoolSize)) * math.ceil(
        ((trainImages.shape[2] - convFiltersW + 1) / maxpoolSize)) * numConvFilter
    softmaxWeights = gen_softmax_weights(numNode, softmaxWeightsLength)
    softmaxBiases = np.zeros(numNode)

    learningRate = 0.005
    avgLoss, accuracy = train(trainImages, trainLabels, learningRate,
                              convFilters, maxpoolSize, softmaxWeights, softmaxBiases)
    print("Average loss: {avgLoss:.3f} | Accuracy: {accuracy:.2f}%".format(
