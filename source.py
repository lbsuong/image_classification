...
'''
    ...
    output_row = h - filtersH + 1
    output_col = w - filtersW + 1

    for row in range(h - filtersH):
        for col in range(w - filtersW):
            # Create a temp array contain value fixing the filter array
            temp_arr = []
            for i in range(filtersH):
                for j in range(filtersW):
                    temp_row = row + i
                    temp_col = col + j
                    idx = temp_row * w + temp_col
                    temp_arr.append(input[idx])
            # Convolution temp array with filter array
            for filter_idx in range(numConvFilter):
                value = 0
                for idx in range(filtersH * filtersW):
                    value = value + temp_arr[idx] * filters[filter_idx][idx]
                output_arr[filter_idx].append(value)

    return output_arr, output_col, output_row
...
def train(trainImages, h, w, trainLabels, learningRate, convFilters, convFiltersH, convFiltersW, maxpoolSize, softmaxWeights, softmaxWeightsLength, softmaxBiases, numNode):
    ...
    for (image, label) in zip(trainImages, trainLabels):
        ...
        maxpoolInputs, maxpoolInputsH, maxpoolInputsW = conv_forward(
            convInput, h, w, convFilters, convFiltersH, convFiltersW, numConvFilter)
        softmaxInputs, softmaxInputsH, softmaxInputsW = maxpool_forward(
            maxpoolInputs, maxpoolInputsH, maxpoolInputsW, numConvFilter, maxpoolSize)
        preSoftmax, postSoftmax = softmax_forward(
            softmaxInputs, softmaxInputsH, softmaxInputsW, numConvFilter, softmaxWeights, softmaxWeightsLength, softmaxBiases, numNode)
        ...
        # Lan truyền ngược.
        gradient = softmax_backprop(
            gradient, learningRate, softmaxWeights, softmaxBiases, preSoftmax)
        gredient = maxpool_backprop(
            gradient, maxpoolInputs, maxpoolInputsH, maxpoolInputsW)
        gradient = conv_backprop(gradient, learningRate, convFilters,
                                 convFiltersH, convFiltersW, numConvFilter, convInput, h, w)
...
def predict(image, h, w, convFilters, convFiltersH, convFiltersW, maxpoolSize, softmaxWeights, softmaxWeightsLength, softmaxBiases, numNode):
    ...
    softmaxInputs, softmaxInputsH, softmaxInputsW = maxpool_forward(
        maxpoolInputs, maxpoolInputsH, maxpoolInputsW, numConvFilter, maxpoolSize)
    _, postSoftmax = softmax_forward(softmaxInputs, softmaxInputsH, softmaxInputsW,
                                     numConvFilter, softmaxWeights, softmaxWeightsLength, softmaxBiases, numNode)
...
def main():
    ...
    numNode = 10
    softmaxWeightsLength = ((h - convFiltersH + 1) //
                            maxpoolSize) * ((w - convFiltersW + 1) // maxpoolSize)
    softmaxWeights = gen_softmax_weights(softmaxWeightsLength, numNode)
    softmaxBiases = [0] * numNode

    learningRate = 0.005
    avgLoss, accuracy = train(trainImages, h, w, learningRate, convFilters, convFiltersH, convFiltersW,
                              numConvFilter, maxpoolSize, softmaxWeights, softmaxWeightsLength, softmaxBiases, numNode)
    print("Average loss: {avgLoss:.3f} | Accuracy: {accuravy:.2f}".format(
        avgLoss=avgLoss, accuracy=accuracy*100))
