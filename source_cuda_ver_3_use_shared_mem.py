from numba import cuda, jit, float64, float32
from numba.cuda.stubs import gridsize, threadIdx
import numpy as np
import math
...
def conv_forward_kernel(input, filters, numImage, output):
    ...
    if node >= output.shape[1] or r >= output.shape[2] or c >= output.shape[3]:
        return
    s_inputs = cuda.shared.array(shape=(32, 32), dtype=float64)
    thread_x = cuda.threadIdx.x
    thread_y = cuda.threadIdx.y
    
    for image in range(numImage):
        if r * input.shape[2] + c < input.shape[1] * input.shape[2]:
            s_inputs[thread_y, thread_x] = input[image, r, c]

        cuda.syncthreads()
        output[image, node, r, c] = 0
        for filterRow in range(filters.shape[1]):
            for filterCol in range(filters.shape[2]):
                output[image, node, r, c] += s_inputs[thread_y + filterRow, thread_x + filterCol] * filters[node, filterRow,
                                                            filterCol]
                # output[image, node, r,
    ...
    #                                c + filterCol] * filters[node, filterRow,
    #                                                         filterCol]
    # inPixelsR = thread_x - filters.shape[1] / 2 + filterRow
    # inPixelsC = thread_y - filters.shape[1] / 2 + filterCol
    # # # inPixelsR = min(max(0, inPixelsR), thread_x)
    # # # inPixelsC = min(max(0, inPixelsC), thread_y)
...
def conv_backprop_kernel(d_L_d_out, numImage, normalizedImage, d_L_d_filters):
    ...
    thread_y = cuda.threadIdx.y
    for image in range(numImage):
        if r * normalizedImage.shape[2] + c < normalizedImage.shape[1] * normalizedImage.shape[2]:
            s_inputs[thread_y, thread_x] = normalizedImage[image, r, c]
        ...
        for d_L_d_out_row in range(d_L_d_out.shape[2]):
            for d_L_d_out_col in range(d_L_d_out.shape[3]):
                d_L_d_filters[image, node, r, c] += d_L_d_out[
                    ...
                    d_L_d_out_col] * s_inputs[d_L_d_out_row + thread_y, d_L_d_out_col + thread_x]
